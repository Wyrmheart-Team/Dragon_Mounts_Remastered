...
public class HatchCountTrigger extends SimpleCriterionTrigger<HatchCountTriggerInstance> {
	...
	public static Criterion<HatchCountTriggerInstance> instance(ContextAwarePredicate player, int count) {
		return ModCriterionTriggers.HATCH_COUNT_TRIGGER.get().createCriterion(new HatchCountTriggerInstance(Optional.of(player), count));
	}
	...
	public record HatchCountTriggerInstance(Optional<ContextAwarePredicate> player, int count)
		...
		public static final Codec<HatchCountTriggerInstance> CODEC = RecordCodecBuilder.create(inst ->
			inst
				.group(
					EntityPredicate.ADVANCEMENT_CODEC.optionalFieldOf("player").forGetter(HatchCountTriggerInstance::player),
