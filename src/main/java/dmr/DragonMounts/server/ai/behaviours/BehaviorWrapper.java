...
public class BehaviorWrapper<E extends LivingEntity> implements BehaviorControl<E> {
    ...
    @SafeVarargs
    public BehaviorWrapper(Predicate<E> shouldStart, Function<TameableDragonEntity, Goal>... behaviors) {
        this(
                Map.of(),
                shouldStart,
                OrderPolicy.SHUFFLED,
                RunningPolicy.RUN_ONE,
                convertToOrderedPairs(
                        Arrays.stream(behaviors).map(GoalWrapper::new).toArray(BehaviorControl[]::new)));
    }

    @SafeVarargs
    public BehaviorWrapper(
            Map<MemoryModuleType<?>, MemoryStatus> entryCondition, BehaviorControl<? super E>... behaviors) {
    ...
    private static <E extends LivingEntity> Pair<BehaviorControl<? super E>, Integer>[] convertToPairs(
            BehaviorControl<? super E>... behaviors) {
        return Stream.of(behaviors)
                .<Pair<BehaviorControl<? super E>, Integer>>map(behavior -> Pair.of(behavior, 1))
                .toArray(Pair[]::new);
    }
    ...
    public boolean tryStart(ServerLevel level, E entity, long gameTime) {
        if ((this.entryCondition == null || this.hasRequiredMemories(entity))
                && (this.shouldStart == null || this.shouldStart.test(entity))) {
