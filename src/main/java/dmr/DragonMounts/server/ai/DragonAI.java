...
public class DragonAI {
    ...
    private static void initCoreActivity(Brain<TameableDragonEntity> brain) {
        brain.addActivity(
                ...
                ImmutableList.of(
                        ...
                        new BehaviorWrapper<>(
                                e -> !e.isRandomlySitting()
                                        || (e.isInLove() && e.getBrain().hasMemoryValue(MemoryModuleType.BREED_TARGET)),
        ...
        brain.addActivity(
                ...
                ImmutableList.of(
                        Pair.of(
                                ...
                                new BehaviorWrapper<>(
                                        Animal::isInLove,
                                        new DragonBreedBehaviour(ModEntities.DRAGON_ENTITY.get(), 1.2F, 6))),
                        Pair.of(
                                0,
                                new BehaviorWrapper<>(
    ...
    public static void initSitActivity(Brain<TameableDragonEntity> brain) {
    ...
    public static void initWanderActivity(Brain<TameableDragonEntity> brain) {
        brain.addActivityWithConditions(
    ...
    private static Optional<? extends LivingEntity> findNearestValidAttackTarget(TameableDragonEntity dragon) {
        return BehaviorUtils.isBreeding(dragon)
                ? Optional.empty()
                : dragon.getBrain().getMemory(MemoryModuleType.NEAREST_ATTACKABLE);
    }
    ...
    private static Optional<PositionTracker> getWanderTarget(LivingEntity entity) {
        if (entity instanceof TameableDragonEntity dragon) {
            if (dragon.hasWanderTarget() && dragon.getWanderTarget().isPresent()) {
                return Optional.of(
                        new BlockPosTracker(dragon.getWanderTarget().get().pos()));
            }
        }
        return Optional.empty();
    }

    public static void updateActivity(TameableDragonEntity dragon) {
    ...
    public static void wasHurtBy(TameableDragonEntity dragon, LivingEntity livingEntity) {
        Brain<TameableDragonEntity> brain = dragon.getBrain();
        brain.eraseMemory(MemoryModuleType.BREED_TARGET);
        maybeRetaliate(dragon, livingEntity);
    }

    public static void maybeRetaliate(TameableDragonEntity dragon, LivingEntity livingEntity) {
