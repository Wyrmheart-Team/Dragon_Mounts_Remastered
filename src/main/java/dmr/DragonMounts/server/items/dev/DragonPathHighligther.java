	...
	public void inventoryTick(ItemStack pStack, Level pLevel, Entity pEntity, int pSlotId, boolean pIsSelected) {
		if (pIsSelected && pEntity instanceof Player pPlayer) {
			if (!pLevel.isClientSide && pPlayer.tickCount % 5 == 0) {
				var dragons = pPlayer.level.getNearbyEntities(
					DMRDragonEntity.class,
					TargetingConditions.forNonCombat().range(128).ignoreLineOfSight(),
					pPlayer,
				...
				for (DMRDragonEntity dragon : dragons) {
					if (!dragon.getNavigation().isDone()) {
						...
						for (int i = dragon.getNavigation().getPath().getNextNodeIndex(); i < nodes - 1; i++) {
							...
							var isAir = dragon.level
								.getBlockState(blockPos.below(2))
								.entityCanStandOn(dragon.level, blockPos.below(2), dragon);
