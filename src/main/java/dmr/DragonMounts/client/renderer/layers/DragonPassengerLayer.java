...
public class DragonPassengerLayer<T extends TameableDragonEntity> extends GeoRenderLayer<T> {
    ...
    public void renderForBone(
        ...
        Entity passenger = entity.getPassengers().size() > passengerNumber
                ? entity.getPassengers().get(passengerNumber)
                : null;
        if (passenger != null) {
    ...
    public <E extends Entity> void renderEntity(
            E entityIn, float partialTicks, PoseStack matrixStack, MultiBufferSource bufferIn, int packedLight) {
        // if(entityIn instanceof DMRDragonEntity) return; //Attempt at fixing crash
        // with EMF mod
    ...
    public static void cancelPassengerRenderEvent(RenderLivingEvent.Pre event) {
        ...
        if (entity.getVehicle() instanceof TameableDragonEntity
                && DragonPassengerLayer.passengers.contains(entity.getUUID())) event.setCanceled(true);
    }
}
