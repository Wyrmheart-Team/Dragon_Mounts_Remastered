...
public class DragonEggRenderer implements BlockEntityRenderer<DragonEggBlockEntity> {
	...
	public void render(DragonEggBlockEntity dragonEggBlockEntity, float v, PoseStack poseStack, MultiBufferSource multiBufferSource, int i, int i1)
	{
		...
		if (model instanceof DragonEggModel.Baked eggModel) {
			...
			poseStack.pushPose(); var time = dragonEggBlockEntity.tickCount; float hatchProgress = ((float)dragonEggBlockEntity.getHatchTime() / dragonEggBlockEntity.getBreed().getHatchTime());
			...
			poseStack.translate(-0.5, 0, -0.5); var renderType =
					RenderTypeHelper.getEntityRenderType(model.getRenderTypes(dragonEggBlockEntity.getBlockState(), dragonEggBlockEntity.getLevel().random, dragonEggBlockEntity.getModelData())
							.asList().getFirst(), true); Minecraft.getInstance().getBlockRenderer().getModelRenderer()
					.renderModel(poseStack.last(), multiBufferSource.getBuffer(renderType), dragonEggBlockEntity.getBlockState(), bakedModel, 0, 0, 0, i, OverlayTexture.NO_OVERLAY);
			poseStack.popPose();
		}
	}
}
