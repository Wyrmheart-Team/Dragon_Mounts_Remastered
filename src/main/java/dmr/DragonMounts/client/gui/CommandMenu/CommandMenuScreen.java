...
public class CommandMenuScreen extends Screen {
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        if (active && CommandOverlayHandler.animationTimer == 0) {
            ...
            // Get the mouse angle.
            double mouseAngle =
                    CommandOverlayHandler.correctAngle(CommandOverlayHandler.getMouseAngle() + (2.5f * ANGLE_PER_ITEM));
            ...
            if (!mc.options.hideGui) {
                for (int i = 0; i < CommandOverlayHandler.MAX_ITEMS; i++) {
                    ...
                    if (mouseDistance < (CommandOverlayHandler.INNER_RADIUS * 1.3f)
                    ...
                    if (mouseAngle >= segStart && mouseAngle < segEnd) {
                        CommandOverlayHandler.MenuItem menuItem =
                                CommandOverlayHandler.getItems()[(i) % CommandOverlayHandler.MAX_ITEMS];
                        if (menuItem != null && menuItem.clickListener() != null) {
                            DMR.LOGGER.debug(
                                    "Clicked on item: {}", menuItem.title().getString());
