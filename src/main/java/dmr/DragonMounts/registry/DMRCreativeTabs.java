...
public class DMRCreativeTabs
{
	...
	public static void init()
	{
		if (DragonMountsRemaster.DEBUG) {
			MOD_HYBRIDS_DEV_TAB = CREATIVE_MODE_TABS.register("dragon_mounts_hybrids_dev", () -> CreativeModeTab.builder().icon(() -> new ItemStack(DMRItems.DRAGON_EGG_BLOCK_ITEM.get())).title(Component.literal("DMR Hybrid eggs")).displayItems((enabledFeatures, entries) -> {
			...
			MOD_TESTING_DEV_TAB = CREATIVE_MODE_TABS.register("dragon_mounts_testing_dev",
			                                                  () -> CreativeModeTab.builder().icon(() -> new ItemStack(DMRItems.DRAGON_EGG_BLOCK_ITEM.get())).title(Component.literal("DMR Testing items")).displayItems((enabledFeatures, entries) -> DMRItems.ITEMS.getEntries().forEach(supplier -> {
