...
public class DMRCreativeTabs {
	...
	public static final Supplier<CreativeModeTab> MOD_TAB = CREATIVE_MODE_TABS.register(
			"dragon_mounts",
			() -> CreativeModeTab.builder().icon(() -> new ItemStack(DMRItems.DRAGON_EGG_BLOCK_ITEM.get())).title(Component.translatable("itemGroup.dragon_mounts"))
					.displayItems((enabledFeatures, entries) -> {
						var breeds = DragonBreedsRegistry.getDragonBreeds(); for (IDragonBreed type : breeds) {
						...
						} var armors = new ArrayList<>(DragonArmorRegistry.getDragonArmors()); armors.sort(Comparator.comparing(DragonArmor::getProtection));
	...
	public static void init()
	{
		if (DragonMountsRemaster.DEBUG) {
			MOD_HYBRIDS_DEV_TAB = CREATIVE_MODE_TABS.register(
					"dragon_mounts_hybrids_dev",
					() -> CreativeModeTab.builder().icon(() -> new ItemStack(DMRItems.DRAGON_EGG_BLOCK_ITEM.get())).title(Component.literal("DMR Hybrid eggs"))
							.displayItems((enabledFeatures, entries) -> {
								var breeds =
										new ArrayList<>(DragonBreedsRegistry.getDragonBreeds()).stream().filter(IDragonBreed::isHybrid).sorted(Comparator.comparing(IDragonBreed::getId)).toList();
			...
			MOD_TESTING_DEV_TAB = CREATIVE_MODE_TABS.register(
					"dragon_mounts_testing_dev",
					() -> CreativeModeTab.builder().icon(() -> new ItemStack(DMRItems.DRAGON_EGG_BLOCK_ITEM.get())).title(Component.literal("DMR Testing items"))
