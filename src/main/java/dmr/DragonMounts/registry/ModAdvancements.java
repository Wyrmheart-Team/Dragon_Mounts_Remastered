	...
	public static void init(ServerLevel level) {
		...
		for (IDragonBreed breed : DragonBreedsRegistry.getDragonBreeds()) {
			...
			var predicate = ItemPredicate.Builder.item()
				...
				.hasComponents(DataComponentPredicate.builder().expect(ModComponents.DRAGON_BREED.get(), breed.getId()).build())
				.build();
		...
		for (IDragonBreed breed : DragonBreedsRegistry.getDragonBreeds()) {
			...
			var hatch = Builder.advancement()
				...
				.addCriterion(
					"hatch_dragon_egg",
					new Criterion<>(ModCriterionTriggers.HATCH_TRIGGER.get(), HatchTrigger.HatchTriggerInstance.test(breed.getId()))
				)
