...
public class DMRTestDumper implements FileSummaryDumper {
	...
	public void write(TestSummary summary, Logger logger, PrintWriter writer) {
		...
		var testsByGroup = summary.testInfos().stream().collect(Collectors.groupingBy(TestInfo::groups, Collectors.toList()));
		...
		testsByGroup.forEach((group, tests) -> {
			Map<Result, List<TestInfo>> testsByStatus = tests
				.stream()
				.collect(
					Collectors.groupingBy(test -> test.status().result(), () -> new EnumMap<>(Test.Result.class), Collectors.toList())
				);
