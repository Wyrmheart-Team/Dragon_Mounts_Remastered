...
/**
 * Tests for the player capability functionality of the mod. These tests verify
 * that player capabilities related to dragons can be properly accessed, set,
 * and managed, including storing and retrieving dragon data.
 */
...
public class CapabilityTests {
     ...
     *
     * <p>
     * This test verifies that: 1. The PlayerStateUtils.getHandler method can
    ...
    /**
     * Tests that the player instance is automatically set in the capability
     * handler.
     *
     * <p>
     * This test verifies that: 1. When retrieving a player's capability handler 2.
     ...
     *
     * <p>
     * This test verifies that: 1. A player's capability handler can be retrieved
     * directly using getData 2. The player instance can be manually set in the
     * handler
     *
     * @param helper
     *               The game test helper
     ...
     *
     * <p>
     * This test verifies that: 1. A dragon entity can be created and added to the
     * level 2. The dragon can be tamed for a specific player 3. The dragon can be
    ...
    /**
     * Tests that setting a dragon to a whistle slot stores the data in the correct
     * slot.
     *
     * <p>
     * This test verifies that: 1. A dragon can be associated with a specific
     * whistle slot (slot 1) 2. After association, the player's capability handler
     * contains exactly one dragon entry 3. The entry is stored with the correct key
    ...
    public static void setDragonSetsCorrectSlot(ExtendedGameTestHelper helper) {
        ...
        if (List.of(handler.dragonNBTs.keys()).size() != 1) {
            helper.fail(
                    "Expected 1 key, got " + List.of(handler.dragonNBTs.keys()).size());
        }
     ...
     *
     * <p>
     * This test verifies that: 1. A dragon can be associated with a whistle slot 2.
     * A new dragon entity can be created from the stored data 3. The created dragon
     * entity is not null
     *
     * @param helper
     *               The game test helper
